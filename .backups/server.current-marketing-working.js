...
let marketingTablePromise = null;

function ensureMarketingBannersTable() {
  if (!marketingTablePromise) {
    marketingTablePromise = pool.query(`
    ...
    `).catch(error => {
      marketingTablePromise = null;
      throw error;
    });
  }

  return marketingTablePromise;
}
...
app.get('/marketing/banners', async (req, res) => {
  try {
    await ensureMarketingBannersTable();

    const result = await pool.query(`
      SELECT id, title, active, sort_order, created_at, updated_at
...
app.get('/marketing/banners/:id/image', async (req, res) => {
  try {
    ...
    return res.send(banner.image_data);
  } catch (error) {
    ...
    try {
      ...
      const result = await pool.query(
        `
          ...
          RETURNING id, title, active, sort_order, created_at, updated_at
        `,
        [title, req.file.buffer, req.file.mimetype, nextOrder, req.user.id]
      );
